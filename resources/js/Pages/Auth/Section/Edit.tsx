...
interface EditProps {
  ...
  export default function Edit({ section, companies, departments }: EditProps) {
    const { data, setData, put, processing, errors, reset } = useForm({
        name: section.name,
        department_id: section.department.id,
        company_id: section.department.company_id
    });
    ...
    const submit: FormEventHandler = (e) => {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                put(route("auth.section.update", section.id));
            }
        });
                ...
                <form onSubmit={submit} className="w-1/3 rounded-xl">
                    <Card className="">
                        <CardHeader>
                            <CardTitle>
                                Edit Section :
                                <span className="text-cyan-600 ml-1">{section.name}</span>
                            </CardTitle>
