...
export default function Model({categories,manufacturers,depreciations,fieldsets,modelDevice}:Props) {
    const { data, setData, processing, errors } = useForm({
        name: modelDevice.name,
        category_id: modelDevice.category_id,
        manufacturer_id: modelDevice.manufacturer_id,
        depreciation_id: modelDevice.depreciation_id || '',
        fieldset_id: modelDevice.fieldset_id || '',
        model_no: modelDevice.model_no,
        min_qty: modelDevice.min_qty,
        notes: modelDevice.notes,
        image: modelDevice.image || null,
    });
    ...
    const submit: FormEventHandler = (e) => {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                router.post(route("auth.model-device.update", modelDevice.id),{
                    _method: "PUT",
                    ...data,
                });
            }
        ...
        <Guest>
            <div className="flex w-full justify-center pt-10">
                <form onSubmit={submit} className="w-full md:w-1/2 rounded-xl">
                    <Card className="">
                        <CardHeader>
                            <CardTitle>Edit Model Asset : {modelDevice.name}</CardTitle>
